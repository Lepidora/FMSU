...
def  main():
  ...
  installer = "\installer\FMSU Installer.exe"
   
  if demo:
    installer = "\installer\FMSU Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\FMSU manual.pdf" 
  ]
  ...
  ZIPNAME = "FMSU-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "FMSU-v" + FULLVERSIONSTR + "-win-demo.zip"
